(function() {
  ...
  // Initialize a pinned timeline that animates full-screen .section_scroll_item slides
  function initTextBackgroundScroll(container) {
    ...
    whenGsapReady(function(){
      ...
      wrappers.forEach((wrapper, index) => {
        const items = wrapper.querySelectorAll('[data-text-bg-scroll-item]');
        ...
        const visuals = Array.from(items).map(item => item.querySelector('[data-text-bg-scroll-visual]'));
        const hasVisuals = visuals.some(v => v !== null);
        if (DEBUG) console.log('[text-bg-scroll] visuals:', visuals.map((v, i) => `item[${i}]: ${v ? v.tagName + '.' + v.className : 'none'}`));
        ...
        if (hasVisuals) {
          visuals.forEach((visual, i) => {
            ...
            if (DEBUG) console.log(`[text-bg-scroll] setting visual[${i}] y:`, -window.innerHeight);
        ...
        */
        ...
        items.forEach((item, i) => {
          ...
          if (DEBUG && visual) console.log(`[text-bg-scroll] wiring visual[${i}]`, visual);
